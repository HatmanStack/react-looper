...
export class FFmpegService implements IAudioExportService {
  ...
  public async mix(options: MixOptions): Promise<Blob> {
    ...
    try {
      ...
      let outputBlob: Blob;
      if (format === "mp3") {
        outputBlob = await this.convertToMP3(wavBlob, quality);
      } else if (format === "m4a") {
   ...
   * Follows official lamejs example implementation
   */
  private async convertToMP3(wavBlob: Blob, quality: string): Promise<Blob> {
    try {
      // Dynamically import lamejs to avoid issues
      ...
      return wavBlob; // Fallback to WAV if conversion fails
    }
  }
}
