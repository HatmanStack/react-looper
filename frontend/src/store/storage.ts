 ...
 * - Web: Uses localStorage (synchronous but wrapped in async for consistency)
 * - Native: Uses @react-native-async-storage/async-storage
 *
 * NOTE: We avoid Zustand's built-in persist middleware because it uses import.meta
 * which causes errors with Metro bundler on React Native. This custom implementation
 * provides the same functionality without the bundler compatibility issues.
 */
