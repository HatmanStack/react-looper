...
describe.skip("SpeedSlider", () => {
  describe("rendering", () => {
    ...
    it("should render minimum speed (0.05x)", () => {
      const { getByText } = render(
        <SpeedSlider value={0.05} onValueChange={jest.fn()} />,
      );
    ...
    it("should render maximum speed as 2.50x", () => {
      const { getByText } = render(
        <SpeedSlider value={2.5} onValueChange={jest.fn()} />,
      );
    ...
    it("should format speed to 2 decimal places", () => {
      const { getByText } = render(
        <SpeedSlider value={1.234567} onValueChange={jest.fn()} />,
      );
  ...
  describe("Android compatibility", () => {
    ...
    it("should display 2.50x for slider value 100 (matches Android 2.44 → 2.50)", () => {
      ...
      const { getByText } = render(
        <SpeedSlider value={100 / 41} onValueChange={jest.fn()} />,
      );
  ...
  describe("edge cases", () => {
    ...
    it("should handle mid-range speeds correctly", () => {
      ...
      speeds.forEach((speed) => {
        const { getByText } = render(
          <SpeedSlider value={speed} onValueChange={jest.fn()} />,
        );
