...
describe('Performance - TrackListItem Rendering', () => {
  ...
  it('should handle rapid play/pause updates', async () => {
    ...
    const benchmark = assertPerformance('Play button update', avgUpdateTime, 50);
    expect(benchmark.passed).toBe(true);
  });
});
...
describe('Performance - Slider Interaction', () => {
  it('should handle volume slider updates in < 50ms', async () => {
    ...
    const benchmark = assertPerformance('Volume slider update interval', avgInterval, 50);
    expect(benchmark.passed).toBe(true);
  });
  ...
  it('should handle speed slider updates in < 50ms', async () => {
    ...
    const benchmark = assertPerformance('Speed slider update interval', avgInterval, 50);
    expect(benchmark.passed).toBe(true);
  });
});
...
describe('Performance - Component Mounting', () => {
  it('should mount TrackListItem quickly', async () => {
    ...
    const benchmark = assertPerformance('TrackListItem mount (avg)', stats.avg, 50);
    expect(benchmark.passed).toBe(true);

    const p95Benchmark = assertPerformance('TrackListItem mount (p95)', stats.p95, 100);
  ...
  it('should mount VolumeSlider quickly', async () => {
    const stats = await runBenchmark(
      'Mount VolumeSlider',
      async () => {
        const { unmount } = render(<VolumeSlider value={75} onValueChange={jest.fn()} />);
        unmount();
      },
      20
    );
    ...
    const benchmark = assertPerformance('VolumeSlider mount (avg)', stats.avg, 30);
    expect(benchmark.passed).toBe(true);
  });
});
...
describe('Performance - List Scrolling Simulation', () => {
  it('should handle rapid track list updates', async () => {
    ...
    const avgDuration = updateDurations.reduce((a, b) => a + b, 0) / updateDurations.length;

    const benchmark = assertPerformance('List update (scrolling simulation)', avgDuration, 100);
    expect(benchmark.passed).toBe(true);
  });
});
