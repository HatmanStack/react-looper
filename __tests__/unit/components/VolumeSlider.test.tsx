...
describe('VolumeSlider', () => {
  describe('rendering', () => {
    it('should render with initial volume value', () => {
      const { getByText } = render(<VolumeSlider value={50} onValueChange={jest.fn()} />);

      expect(getByText('Volume: 50')).toBeTruthy();
    });

    it('should render minimum volume (0)', () => {
      const { getByText } = render(<VolumeSlider value={0} onValueChange={jest.fn()} />);

      expect(getByText('Volume: 0')).toBeTruthy();
    });

    it('should render maximum volume (100)', () => {
      const { getByText } = render(<VolumeSlider value={100} onValueChange={jest.fn()} />);

      expect(getByText('Volume: 100')).toBeTruthy();
    });

    it('should round volume display to integer', () => {
      const { getByText } = render(<VolumeSlider value={75.7} onValueChange={jest.fn()} />);
  ...
  describe('volume range', () => {
    it('should have slider min value of 0', () => {
      const { getByTestId } = render(<VolumeSlider value={50} onValueChange={jest.fn()} />);
  ...
  describe('interaction', () => {
    it('should call onValueChange when slider moves', () => {
      ...
      const { getByTestId } = render(<VolumeSlider value={50} onValueChange={onValueChange} />);
  ...
  describe('edge cases', () => {
    ...
    it('should handle fractional volumes by rounding for display', () => {
      ...
      testCases.forEach(({ value, expected }) => {
        const { getByText } = render(<VolumeSlider value={value} onValueChange={jest.fn()} />);
  ...
  describe('Android compatibility', () => {
    ...
    it('should display integer values like Android', () => {
      ...
      const { getByText } = render(<VolumeSlider value={42} onValueChange={jest.fn()} />);
