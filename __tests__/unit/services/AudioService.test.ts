...
describe("AudioService", () => {
  ...
  describe("Playback Operations", () => {
    ...
    it("enforces max concurrent players limit", async () => {
      ...
      await expect(audioService.loadTrack("track4", mockUri)).rejects.toThrow(
        AudioError,
      );
    });
  ...
  describe("File Operations", () => {
    it("copies file to app storage", async () => {
      const result = await audioService.copyToAppStorage(
    ...
    it("generates unique filename", () => {
      const filename1 = audioService.generateUniqueFilename(
        "test",
        AudioFormat.MP3,
      );
      const filename2 = audioService.generateUniqueFilename(
        "test",
        AudioFormat.MP3,
      );
