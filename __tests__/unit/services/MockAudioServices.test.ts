...
describe('MockFileManager', () => {
  ...
  it('saves audio file', async () => {
    const uri = await fileManager.saveAudioFile('data', 'test', AudioFormat.MP3);
    expect(uri).toBeTruthy();
  ...
  it('exports to external storage', async () => {
    const uri = await fileManager.saveAudioFile('data', 'test', AudioFormat.MP3);
    const exportUri = await fileManager.exportToExternalStorage(uri, 'exported');
  ...
  it('generates unique filename', () => {
    const filename1 = fileManager.generateUniqueFilename('test', AudioFormat.MP3);
    const filename2 = fileManager.generateUniqueFilename('test', AudioFormat.MP3);
  ...
  it('cleans up temp files', async () => {
    // Save a temp file
    const tempUri = await fileManager.saveAudioFile('data', 'temp-file', AudioFormat.MP3);
