...
describe('Migration System', () => {
  describe('runMigrations', () => {
    ...
    it('should run multiple migrations sequentially', () => {
      ...
      expect(migration3).toHaveBeenCalledWith({ initial: true, v1: true, v2: true });
    });
  ...
  describe('createVersionedState', () => {
    it('should create versioned state wrapper', () => {
      ...
      expect(versioned.versionHistory![0].timestamp).toBeLessThanOrEqual(Date.now());
    });
    ...
    it('should not duplicate version in history', () => {
      ...
      expect(versioned.versionHistory!.filter((h) => h.version === 2).length).toBe(1);
    });
  });
